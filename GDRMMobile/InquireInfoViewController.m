...
-(void)loadCaseInfoArray{
    ...
    if (caseInfo) {
        ...
        localityString=caseInfo.short_happen_place;
...
-(NSString*)generateCommonInquireText{
    ...
    CaseInfo *caseInfo = [CaseInfo caseInfoForID:_caseID];
...
//        Citizen *citzen = [Citizen citizenForName:self.textParty.text nexus:self.textNexus.text case:self.caseID];
        Citizen *citzen=[Citizen citizenForCitizenName:self.textParty.text nexus:self.textNexus.text case:self.caseID];
        caseStatusString =  [NSString stringWithFormat:@"车辆%@损坏",citzen.bad_desc];
    } else {
        caseStatusString = @"未造成车辆损坏";
    }
    
    NSString* text = [NSString stringWithFormat:@"%@%@%@%@%@%@%@%@%@",@"问：你好!我们是",organizationName,@"路政员，现依法对你进行询问。请你如实回答我们的询问并协助调查，不得隐瞒和弄虚作假，否则将承担法律责任，你有权对有关情况作陈述和申辩，你听清楚没有？\n答：听清楚了。\n问：你是由于什么原因发生交通事故的？\n答：因",caseInfo.case_reason,@"导致车辆失控。\n问：事故是否造成人员伤亡，车辆是否造成损坏及损坏程度如何？\n答：",[CaseProveInfo generateWoundDesc2:self.caseID],@"，",caseStatusString,@"。\n问：事故造成公路路产损坏，损坏路产详见公路赔（补）偿清单。你是否认可？\n答：认可。\n问：以上内容是否属实？\n答：属实。"];
    return text;
}
@end
