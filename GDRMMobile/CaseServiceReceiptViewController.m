...
- (void)pageLoadInfo{
    ...
    self.textMark2.text = [NSString stringWithFormat:@"%@%@",[FileCode fileCodeWithPredicateFormat:@"赔补偿案件编号"].organization_code,@"赔" ];//caseInfo.case_mark2;
    self.textMark3.text = [NSString stringWithFormat:@"%@-%@", caseInfo.case_mark2,caseInfo.full_case_mark3];
    self.textincepter_name.text = self.caseServiceReceipt.incepter_name;
    self.textreason.text = [NSString stringWithFormat:@"%@%@%@", citizen.automobile_number, citizen.automobile_pattern, proveInfo.case_short_desc];
    ...
    if (self.caseServiceReceipt.remark != nil && ![self.caseServiceReceipt.remark isEmpty]){
		...
		self.textremark.text = @"事故现场送达，当事人签收。";
	}
...
- (void)generateDefaultInfo:(CaseServiceReceipt *)caseServiceReceipt{
    ...
    caseServiceReceipt.service_position = caseInfo.short_happen_place;
