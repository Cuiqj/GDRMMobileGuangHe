...
- (NSString *)station_end_km{
    return [[NSString alloc] initWithFormat:@"%02d",self.station_end.integerValue/1000];
}
- (NSString *)station_end_m{
    return [[NSString alloc] initWithFormat:@"%03d",self.station_end.integerValue%1000];
}
- (NSString *)roadsegment{
    return [RoadSegment roadNameFromSegment:self.roadsegment_id];
}
- (NSString *)side_short{
...
- (NSString *)full_case_mark4{
    ...
    return [NSString stringWithFormat:@"%@-%@%@", self.case_mark2,lochus_code, self.case_mark3];
...
- (NSString *)organization_label2
{
    return [NSString stringWithFormat:@"%@%@",[FileCode fileCodeWithPredicateFormat:@"赔补偿案件编号"].organization_code,@"赔" ];
}
@end
