...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.state == kRecord) {
        ...
        //如果描述的的内容只有时间（长度为5）则不显示
        if (record.remark.length<=5) {
            cell.labelRemark.text=nil;
        }else{
        ...
        //如果起始位置和结束位置都没确定则不现实
        if ((stationStartM==nil&&stationStartKM==nil)) {
            cell.labelStation.hidden=YES;
            cell.labelStation.text=nil;
