...
+ (NSString *)generateEventDescForCase:(NSString *)caseID{
    ...
    if (citizenArray.count>0) {
        if (citizenArray.count==1) {
            ...
            caseDescString=[caseDescString stringByAppendingFormat:@"   %@于%@驾驶%@%@行至%@%@%@，在公路%@行驶过程中，由于%@发生交通事故，并造成公路路产损坏，事故中%@。经与当事人现场勘查，",citizen.party,happenDate,citizen.automobile_number,citizen.automobile_pattern,roadName,caseInfo.side,stationString,caseInfo.place,caseInfo.case_reason,caseStatusString];
            ...
            if (deformArray.count>0) {
                ...
                caseDescString=[caseDescString stringByAppendingFormat:@"损坏公路路产如下：%@。",deformsString];
            } else {
...
+ (NSString *)generateEventDescForCase2:(NSString *)caseID{
    ...
    if (citizenArray.count>0) {
        if (citizenArray.count==1) {
            ...
            caseDescString=[caseDescString stringByAppendingFormat:@"%@于%@驾驶%@%@行至%@%@%@在公路%@由于%@发生交通事故损坏公路路产，%@，经与当事人现场勘查，损坏公路路产见公路赔（补）偿清单。",citizen.party,happenDate,citizen.automobile_number,citizen.automobile_pattern,roadName,caseInfo.side,stationString,caseInfo.place,caseInfo.case_reason,caseStatusString];
        ...
        if (citizenArray.count>1) {
            ...
            caseDescString=[caseDescString stringByAppendingFormat:@"在公路%@由于%@发生碰撞，造成交通事故，%@，经与当事人现场勘查，损坏公路路产见公路赔（补）偿清单。",caseInfo.place,caseInfo.case_reason,caseStatusString];
                    }
    }
    return caseDescString;
}
...
- (NSString *) citizen_org_duty{
    ...
    if ([citizen.org_name isEmpty]&&[citizen.org_principal_duty isEmpty]) {
        return nil;
    }
...
+ (NSString *)generateWoundDesc2:(NSString *)caseID{
    CaseInfo *caseInfo = [CaseInfo caseInfoForID:caseID];
